...
/**
* Clase Manejadora Tasoka_vacunas
* 
...
class Tasoka_vacunas extends gvHidraForm_DB
{
	public function __construct() {
		...
		$nombreTablas= array('tasoka_vacuna');
		parent::__construct($g_dsn, $nombreTablas);

		/************************ QUERYs ************************/
		
		//Consulta del modo de trabajo LIS
		$str_select = "SELECT id_vacuna as \"lis_id_vacuna\", tipo_animal as \"lis_tipo_animal\", nombre_vacuna as \"lis_nombre_vacuna\" FROM tasoka_vacuna";
		...
		//Modo de trabajo FIL
		$this->addMatching("fil_id_vacuna","id_vacuna","tasoka_vacuna");
		$this->addMatching("fil_tipo_animal","tipo_animal","tasoka_vacuna");
		$this->addMatching("fil_nombre_vacuna","nombre_vacuna","tasoka_vacuna");

		//Modo de trabajo LIS
		$this->addMatching("lis_id_vacuna","id_vacuna","tasoka_vacuna");
		$this->addMatching("lis_tipo_animal","tipo_animal","tasoka_vacuna");
		$this->addMatching("lis_nombre_vacuna","nombre_vacuna","tasoka_vacuna");
		...
		$this->addFieldType('fil_nombre_vacuna',$string);
		$string = new gvHidraString(false, 50);
		$string->setRequired(true);
		$this->addFieldType('lis_nombre_vacuna',$string);
		

		//Integers: gvHidraInteger type
		$int = new gvHidraInteger(false, 1);
		$this->addFieldType('fil_id_vacuna',$int);
		$int = new gvHidraInteger(true, 1);
		$int->setRequired(false);
		$this->addFieldType('lis_id_vacuna',$int);
	...
	/*	$int = new gvHidraInteger(false, 1);
		...
		*/
	    ...
	    $lista_raza_lis = new gvHidraList('lis_tipo_animal',"ESPECIES");
		$lista_raza_lis->addOption('','Selecciona...');
		$lista_raza_lis->addOption('T','Todas');
	...
	*/		
	public function preInsertar($objDatos) {
		$id=$this->calcularSecuencia('tasoka_vacuna','id_vacuna',array(),1);
		$m_datos=$objDatos->getAllTuplas();
		foreach($m_datos as $index=>$registro){
		    $m_datos[$index]['lis_id_vacuna']=$id++;
		}
