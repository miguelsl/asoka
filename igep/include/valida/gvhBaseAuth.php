...
abstract class gvhBaseAuth extends Auth_Container {
	...
	function open($url, $https=null, $auth=null)
	{
		...
		if (is_null($https))
			...
			$redir = ($https? 'https': 'http').'://'.$_SERVER['SERVER_NAME'].($https? ':'.$_SERVER['SERVER_PORT']: '').dirname($_SERVER['PHP_SELF']).'/';
		header('location: '.$redir.'openApp.php?option='.urlencode($url).'&login='.urlencode($login));
	}
}
