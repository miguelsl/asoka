...
class IgepSmarty
{
	...
	{		
		...
		$arrayJS = "valorMult = new Array(); valorMult=[";
		if (is_array($valor))
		{
			for($i=0;$i<count($valor);$i++)
			{
			$opc = IgepSmarty::escapeIGEP($valor[$i]);
			$arrayJS .= "desescapeIGEPjs('".$opc."')";
			if ($i<(count($valor)-1))
			$arrayJS .= ",";
			}
			$arrayJS .= "];";
		...
		$cadenaJs .= "		//setSelectedMultipleOption(formulario,'$nombreCampoDestino', desescapeIGEPjs(\"".IgepSmarty::escapeIGEP($valor)."\"));
							setSelectedMultipleOption(formulario,'$nombreCampoDestino', valorMult);
						break;
						case 'radio':
