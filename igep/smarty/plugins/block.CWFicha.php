...
{
	////////////////////////////////////////////////////////////////////////////////
	// INICIALIZACIï¿½N DE VALORES EN LA PILA //
	////////////////////////////////////////////////////////////////////////////////
	
	//Puntero a la pila de etiquetas que contiene a CWFicha 
	$indicePila = count($smarty->_tag_stack)-1;
	//Puntero a la etiqueta Padre (CWFichaEdicion o CWContenedor si es una bï¿½squeda) 
	$punteroPilaPadre = $indicePila - 1;
	$CWPadre = $smarty->_tag_stack[$punteroPilaPadre][0];
	
	//Puntero a la etiqueta Abuelo (CWContenedor o CWPanel si es una bï¿½squeda) 
	...
	// ITERACIï¿½N ACTUAL.- posicion [2] de la pila de CWFicha
	...
	if (!isset($content)) 
	{
		...
		$n_comp="CWFicha";	
		// Necesitamos saber cuï¿½ntas instancias de este componente existen ya / para poner el codigo o no
	...
	{	
		...
		{	
			...
			$numPaginas = 1;
			if ($CWPadre == "CWContenedor") //Si soy un panel de bï¿½squeda...
			{
			...
			if ($pagActual==1) //Primera pï¿½gina
			{						
			...
			//REVIEW: Vero y David. Revisar cï¿½digo
		  	if ($CWPadre == "CWContenedor") //Si soy un panel de bï¿½squeda...
		  	{
		...
		{					
			...
			else 
			{
				...
				//if (($pagTotales == 0) && ($pagActual == 1) && (($idPanel == 'lisDetalle') && ($tresModos != 1 )))					
				{
					$style = "display: block;
							padding-top: 50px;
							width: 100%;
		...
		// if (Es panel de bï¿½squeda) && (no hay solapas)
		{ 
			// Para cuando la ficha esta dentro d otro componente, mostramos el contenido completo
			// Ahora se utiliza para tener una ficha en el panel de bï¿½squeda, donde el padre es CWContenedor
			$resultado = $content;
		}	  	  
	} // else (!isset($content))		
