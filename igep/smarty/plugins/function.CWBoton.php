	...
	switch ($accion) 
	{
		...
		case 'volver' :
			...
			else
			{
				...
				$funcion = "confirm.set('confirm','capaAviso',";
				$funcion .= "'".$tipo."',";
				$funcion .= "'".$codError."',";
				$funcion .= "desescapeIGEPjs('".$descBreve."'),";
				// Necesitamos pasar el action del destino
				$funcion .= "desescapeIGEPjs('".$textoAviso."'),'No','Si',this.form.name,'".$action."');";
				$funcion .= "confirm.mostrarAviso();\n";
			}
			if (!$forzarVisibilidad) $visibilidad = 'inline'; //Accion por defecto según patrón
		break;
		///////////////////
