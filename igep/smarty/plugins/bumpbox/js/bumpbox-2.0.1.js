/* <![CDATA[ */ 
function doBump(names, pathCustom, inSpeed, outSpeed, effectsIn, effectsOut) {

	boxColor = '111'; 
	bgImage = 'igep/smarty/plugins/bumpbox/images/demo_bg_bumpbox.png';
	bgPosition = 'top left';
	bgRepeat = 'repeat-x';	
	borderWeight = 1;
	borderColor = '303132';	
    ...
    /*if (!bgImage) {
    	bgImage = pathCustom+'images/bumpbox/ajax-loader.gif';
    }*/
    //var iconPath = pathCustom+'images/bumpbox/iconsets/' + iconSet + '/';
    var iconPath = pathCustom+'images/bumpbox/iconsets/';
    ...
    $$(names).addEvent('click', function (e) {
        ...
        if (maxw == 0) {
            maxw = 600; //jorge, maximo por defecto cuando no llega rel
        }
        if (maxh == 0) {
            maxh = 600; //jorge, maximo por defecto cuando no llega rel
        }
        if (content.indexOf(".jpg") != -1 || content.indexOf(".gif") != -1 || content.indexOf(".png") != -1) {
            ...
            //escalo la imagen para tener el mismo ratio
           if (w > maxw || h > maxh) {
              if (w < h) {
                h = (h * maxw) / w;
                w = maxw;
		...
		// jorge, cambio el width de la imagen para ajustarlo al maximo definido en el rel
	    img.width=w;
	    img.height=h;
            //maxw = img.width;
            //maxh = img.height;
        ...
        var el = new Element('div', {'class' : 'growBumpbox', 'id': 'grow'});
        ...
        var bg = new Element('div', {'class' : 'bgBumpbox', 'id': 'bg'});
        $(bg).setStyles({
            'top': t
        });
            ...
            onComplete: function () {
               ...
               /* $('grow').setStyle('-moz-box-shadow', '3px 3px ' + boxShadowSize + 'px #' + boxShadowColor);
                ...
                if (content.indexOf(".jpg") != -1 || content.indexOf(".gif") != -1 || content.indexOf(".png") != -1) {
                    ...
                    img.src = content;
			//seteamos el tamaño de la imagen al maximo definido
                    img.width=maxw;
                    img.height=maxh;
                    ...
                    div.inject(el);
                    var f = flowplayer(div, "igep/smarty/plugins/bumpbox/flowplayer/flowplayer.swf", {
                        plugins: {
                            viral: {
                                url: 'igep/smarty/plugins/bumpbox/flowplayer/flowplayer.viralvideos-3.2.3.swf',
                                embed: {
                    ...
                    div.inject(el);
                    f = flowplayer(div, "igep/smarty/plugins/bumpbox/flowplayer/flowplayer.swf", {
                        buffering: true,
                        autoplay: true,
                        clip: content,
                        wmode: 'transparent',
                        plugins: {
                            ...
                            myContent: {
                                url: 'igep/smarty/plugins/bumpbox/flowplayer/flowplayer.audio.swf',
                                bottom: 0,
                                width: maxw - 20,
                                left: 0,
                                borderRadius: 10,
