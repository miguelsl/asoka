...
//			$cadHTML .= "<label class='text' >";
	...
	else
	{
		...
		$cadHTML.="<select name=\"$nameCampo\" id=\"$idCampo\" ".$html_numCaracteres." ".$size." ".$multiple." ".$classHTML." ".$tabindex." ".$estadoHTML." ".$igepSmarty->getAcciones();
	...
	else 
	{
		...
		{
			$numValores = count($v_datos['lista']);	
			$campoHidden.= "<input type='hidden' name='l".$nameCampo."' id='l".$idCampo."' ".$classHTML." value='";
			$valorHidden = '';		
			for($i=0; $i < $numValores; $i++)
			{
				$cadOpcion.="<option ".$classHTML." ";
