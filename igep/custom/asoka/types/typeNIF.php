 ...
 *
 *
 * @version $Id: typeNIF.php,v 1.15 2013/05/10 11:13:20 afelixf Exp $
 * 
	...
	private function valida_nif_cif_nie($cif) {
        ...
        if($this->validateNIE){
            ...
            if (preg_match('/^[TXYZ]{1}/',$cif)) {  
                ...
                if ($num[8]==substr('TRWAGMYFPDXBNJZSQVHLCKE',substr(preg_replace(array('/X/','/Y/','/Z/'),array('0','1','2'),$cif),0,8)%23,1) || preg_match('/^[T]{1}[A-Z0-9]{8}$/',$cif)) {return 0;} else {throw new Exception('No es un NIE válido.');}}
        }
        //Si es un NIF
        if($this->validateNIF){
