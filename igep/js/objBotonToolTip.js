...
* global y de ella heredarán las distintas clases de botones posteriormente 
*/
...
//Devuelve el obj HTML Form, formulario donde se encuentra el botón
...
objBotonToolTip.method('constructor', function (nomObjeto,idPanel,esMaestro,esDetalle)
	{
		...
		/* Sólo se utilizan en las pantallas con Maestro Detalle */
		this.idPanelMD = null;
...
//// getFormId
//// -------------
//Devuelve una cadena que será el nombre del formulario donde se encuentra el botón
...
//// getPanel
//// ------------
//Devuelve el nombre del panel donde se encuentra el botón
...
//// getNombreObj
//// -------------------
//Devuelve una cadena que será el nombre de la instancia creada
...
//// estoyEnMaestro
//// --------------------
//Devuelve un true o false según este o no en un panel maestro
...
//// estoyEnDetalle
//// --------------------
//Devuelve un true o false según este o no en un panel detalle
...
objBotonToolTip.method('deshabilitarBoton', function ()
	{
		//Si el boton ya está deshabilitado SALIMOS 
...
objBotonToolTip.method('habilitarBoton', function ()
	{
		//Si el boton ya está habilitado SALIMOS 
		if (this.habilitado) return;
		ruta = this.imagen.src;
...
objBotonToolTip.method('deshabilitarMD', function ()
	{
		...
		for (i=0; i<v_botones.length; i++)
		{
			...
			{
				// Si hay nDetalles deshabilitar las pestañas
...
//// getPaginaActiva
//// --------------------
//Devuelve un entero que indica la página activa
...

////////// 	CLASE PARA EL botón LIMPIAR.- objBTTLimpiar 							

function objBTTLimpiar (nomObjeto,idPanel,esMaestro,esDetalle)
{
...
objBTTLimpiar.method('limpiarCampos', function ()
{
	...
	//Determinamos el estado (inserción o modificación)
	...
	if ((accionActiva == "insertada") || (eval('formulario.accionActivaP_F_'+this.idPanel+'.value == "insertar"')))
		operacion = "reset"; //Si es una inserción, hacemos reset
	...
	switch (this.idPanel)
	{
		...
		case 'ediMaestro':
			for(i=0; i<formulario.length; i++)
			{
				...
				if (vIdElemento.length > 2) 
					...
					prefijo =''; //Estamos tratando el panel de búsqueda ¿dificil no?
									
				// sólo queremos actuar sobre los campos de usuario, es decir, eliminamos:
					...
					)
				{
					...
					// Encontramos un campo de la página activa
					if (pagCampo == pagActiva)
					{
						if (prefijo=='ins') 
						{
							//Si es una inserción no hay valor anterior
						...
						{
							////////////////////////////////////////
							// opción volver a los valores anteriores
								nombreSinPrefijo = idElemento.substr(3,idElemento.length);
								if ((elemento.type != 'select-one') && 	(elemento.type != 'select-multiple'))
								{
									...
									{
										nameElemento = elemento.name;
										numRadios = eval('formulario.'+nameElemento+'.length'); 
										nameSinPrefijo = nameElemento.substr(3,nameElemento.length);					
										value = eval('formulario.ant'+nameSinPrefijo+'.value');
										vName = nameElemento.split('___');
										for(j=0;j<numRadios;j++)
										{
											idRadio = 'cam___'+vName[1]+j+'___'+vName[2];
											if (eval('formulario.'+idRadio+'.value') == value)
											{
												eval('formulario.'+idRadio+'.checked = true');
												break;
											}
		...
		case 'lisMaestro':
				if (operacion=='reset')
				{ // ESTAMOS EN MODO INSERCIÓN, POR LO TANTO SOLAMENTE LIMPIAREMOS LOS CAMPOS DE INSERCIÓN	
					for(i=0;i<formulario.length;i++)
					{				
				...
				{ // ESTAMOS EN MODO EDICIÓN, NECESITAMOS EL idFila
					idFila = '';
					for(i=0;i<formulario.length;i++)
					{				
						elemento = eval('formulario.elements[i]');
						...
						if (idFila != '')
						{
							...
							vIdElemento = idElemento.split('___');
							// Sólo queremos los posibles campos q puede haber en una tabla
							// los campos d la fila seleccionada y sólo los visibles				
...

//////// 	CLASE PARA EL botón INSERTAR.- objBTTInsertar						

function objBTTInsertar (nomObjeto,idPanel,esMaestro,esDetalle, tresModos)
{
	//llamamos al constructor del padre
	this.uber('constructor',nomObjeto,idPanel,esMaestro,esDetalle);
...
// activarCampos para el botón insertar
	...
	// Nombre campo: 'prefijoCampo___'
		...
		for(i=0;i<formulario.length;i++)
		{
			...
			if ((idCampo[0] == 'cins') || (idCampo[0] == 'ins'))
			{
				...
				//según el valor del CLASS inicial fijamos el nuevo valor
				nuevoClass = '';
...
objBTTInsertar.method('insertar', function ()
	{
		...
		// Campo oculto para indicar a la parte de negocio la acción q se va a realizar
...
/*  En principio esta función ya no se utiliza, para insertar directamente desde el panel 
	de búsqueda se hace un submit con accion='nuevo' */	
objBTTInsertar.method('insertarEnBusqueda', function (panelActivar)
	{
/*		if (this.habilitado ==  false) return;
		
		//Activamos los botones inferiores!!
		this.activarGC(panelActivar);
		
		// Establecemos el formulario que queremos activar, corresponde al panel en el q aparecer?n los campos para insertar
		this.formulario = eval('document.getElementById("F_'+panelActivar+'")');
		formulario = this.getForm();
		
	 	// Campo oculto para indicar a la parte de negocio la acción q se va a realizar
		accionActivaP = 'accionActivaP_F_'+panelActivar;
		eval('formulario.'+accionActivaP+'.value="insertar"');
		eval('document.getElementById("accionActiva").value="insertada"');
		
		// Se tiene que desactivar el panel de filtro y activar el correspondiente donde se va a insertar
		eval('document.getElementById("P_fil").style.display = "none"');
		eval('document.getElementById("P_'+panelActivar+'").style.display = "block"');		
		...
		{
			idCapa = capas[i].id;
			if ( (idCapa.substring(0,4) == 'pag_') )  // Ocultamos todas las capas correspondientes a páginas
				eval('document.getElementById("'+idCapa+'").style.display = "none"');
			i++;
		}
//alert('función insertarEnBusqueda, panel a activar: '+panelActivar);		
//		eval('document.getElementById("pag_'+panelActivar+'_999").style.display = "block"');
		
		// Activamos los campos donde se insertará el nuevo registro
		this.activarCampos('ins');
		this.habilitado = false;*/
	}
);
...
//************************************************************************//
//*** FUNCIÓN: obtenerValoresMaestro(maestro, detalle) 			 ****/
...
objBTTInsertar.method('obtenerValoresMaestro', function (maestro, detalle)
{
	...
	for (i=0;i<maestro.length;i++)
	{
		...
		if ((maestro.elements[i].type == 'checkbox') && (maestro.elements[i].checked) ) 
		{
			...
			{
				idFila = vNombre[2]; // El número d fila chequeada
...
//////////////////////////////////////////////////////
// Función para el plugin CWSelector
// Parámetros: 
//			- destino: id de la lista múltiple q será donde se vayan recogiendo todos los valores de los campos
//			- separador: carácter que se utilizará de separador cuando haya más de un campo a acumular por cada línea de la lista múltiple destino
//			- resto de argumentos, no aparecen indicados literalmente, pero se le pasarán los id de todos los campos origen de los que se copiar?n
//				los valores a la lista múltiple destino
//////////////////////////////////////////////////////
objBTTInsertar.method('copiarToLista', function (destino, separador)
	{
	    ...
	    if ( (arguments.length == 3) && (eval('document.getElementById("'+arguments[2]+'").type') == 'select-multiple') )
	    {
			...
			for (var j=0;j<select.options.length;j++)
			{
				...
				{
					// Añadimos el valor de la concatenación de todos los campos al select múltiple destino
	    ...
	    {
		    for(i=2;i<arguments.length;i++)  // Bucle q comienza en 2 pq solo queremos recorrer los campos que actúan de origen
			{
				...
				// Hay más de un campo origen y aún no estamos en el último por lo tanto necesitamos separador
...

//////// 	CLASE PARA EL botón MODIFICAR.- objBTTModificar						

function objBTTModificar (nomObjeto,idPanel,esMaestro,esDetalle,tresModos)
{
	//llamamos al constructor del padre
	this.uber('constructor',nomObjeto,idPanel,esMaestro,esDetalle);
...
objBTTModificar.method('modificarTabla', function ()
	{
		...
		formulario = this.getForm();
		nomForm = this.getFormId();
		// Tenemos un campo oculto q contendrá la acción realizada con los botones ToolTip
		accionActivaP = 'accionActivaP_'+nomForm;
		...
		for(i=0;i<formulario.length;i++) 
		{
			...
			{
						// Las filas chequeadas se marcarán como modificadas
		...
		if (eval(this.idPanel+'_paginacion'))
		{
			...
			//Se marca la página como modificada...
...
//Añadido para controlar error JS
		//classCampo = "modify";
//Fin añadido
		for (i=0;i<formulario.length;i++) {
	  		...
	  		{
/// Necesario para el caso de q el campo sea una lista de selección múltiple
	 			...
	 			if (campo[2] == pagina) 
	 			{
	  				...
	  				if (eval('document.getElementById("jump_'+campo[1]+"___"+campo[2]+'")'))
					{
						...
						if (src.indexOf("off.gif") != -1)
							src = src.replace("off.gif",".gif");
						//else
						//	src = src.replace("pestanyas/pix_trans.gif","botones/39.gif");
	  				...
	  				if (eval('document.getElementById("func_'+campo[1]+"___"+campo[2]+'")'))
					{
						...
						if (src.indexOf("off.gif") != -1) // Está en off
							src = src.replace("off.gif",".gif"); // Está en on
						//else
						//	src = src.replace("pestanyas/pix_trans.gif","botones/39.gif");
...

//////// 	CLASE PARA EL botón ELIMINAR.- objBTTEliminar						

function objBTTEliminar(nomObjeto,idPanel,esMaestro,esDetalle)
{
	//llamamos al constructor del padre
	this.uber('constructor',nomObjeto,idPanel,esMaestro,esDetalle);
...
//////////////////////////////////////////////////////
// Función para el objeto CWSelector
//////////////////////////////////////////////////////
...

////////// 	CLASE PARA EL botón EXPANDIR del arbol.- objBTTArbol

