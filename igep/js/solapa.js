...
{
	//if (solapaActivada.className == "opcion") //Si NO está activada...
	...
	{	
		...
		for (var i=0; i<numPags; i++)//Recorremos los id de las solapas de Datos
		{
			for(var posicion=0; posicion<this.cantSolapas; posicion++) // Recorremos las solapas de esa ficha
			{
				capaTxt = eval('document.getElementById("solTxt__'+idFicha+'__'+i+'__'+posicion+'")');
				capaCont = eval('document.getElementById("solCont__'+idFicha+'__'+i+'__'+posicion+'")');
				capaEsq = '';
				if  (eval('document.getElementById("solEsq__'+idFicha+'__'+i+'__'+posicion+'")'))
					capaEsq = eval('document.getElementById("solEsq__'+idFicha+'__'+i+'__'+posicion+'")');		
				capaData = eval('document.getElementById("solData__'+idFicha+'__'+i+'__'+posicion+'")');
