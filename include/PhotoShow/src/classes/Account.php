...
class Account extends Page
{
	...
	public function __construct($usuario){
	    ...
	    $this->login	= (string)$usuario['login'];
				//$this->password = (string)$account->password;
		$this->name		= (string)$usuario['nombre'];
		$this->email	= '';//(string)$account->email;
		$this->language	= 'es';//(string)$account->language;
		//$this->key		= (string)$account->key;

		$this->groups 	= array();

		/*foreach($usuario['modulos']['ciudad'] as $group){
					$this->groups[] = (string)$group;
		}
		*/
		$this->groups[]=(string)$usuario['rol'];
		if(isset($usuario['multimedia']) && $usuario['multimedia']==='S')
		$this->groups[]='admin';
		//throw new Exception("Login $login not found");
		if(!isset($usuario['activo']) || $usuario['activo']!=='S')
		  return false;
	}
	...
	*/
	...
	private function key(){
		...
		return substr(sha1($this->name.$salt),0,5);
	}
